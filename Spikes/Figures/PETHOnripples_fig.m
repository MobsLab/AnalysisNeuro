%% Parameters
expeName = 'UMazePAG'; %'UMazePAG';
IsSave=1;
switch expeName
    case 'UMazePAG'
        mice = [797 798 828 861 882 905 906 911 912 977 994 1117 1161 1162 1168];
    case 'StimMFBWake'
        mice = [117 161];
end
wi = 0.5; % in sec
...
for imouse = 1:length(Dir.path)
    ...
    load([Dir.path{imouse}{1} '/Ripples.mat'], 'ripples');
    % patch different ripples variable
    if exist('ripples','var')
        r{imouse}.ripples = ripples;
...
UseForTresh = SustVal;
[~,ind1] = sort(UseForTresh);
SZ_toplot{2} = SZz(ind1,:);
SZz_norip = zscore(SZ_norip{2}')';
SZ_norip_toplot{2} = SZz_norip(ind1,:);
% Other place cells
OPCz = zscore(OPC{2}')';
SustVal = nanmean(OPCz(:,40:65),2);
UseForTresh = SustVal;
[~,ind2] = sort(UseForTresh);
OPC_toplot{2} = OPCz(ind2,:);
OPCz_norip = zscore(OPC_norip{2}')';
OPC_norip_toplot{2} = OPCz_norip(ind2,:);
% Other cells
OCCz = zscore(OCC{2}')';
SustVal = nanmean(OCCz(:,40:65),2);
UseForTresh = SustVal;
[~,ind3] = sort(UseForTresh);
OCC_toplot{2} = OCCz(ind3,:);
OCCz_norip = zscore(OCC_norip{2}')';
OCC_norip_toplot{2} = OCCz_norip(ind3,:);

% Sort others on conditioninig
for isess = [1 3]
    % Shock
    SZz = zscore(SZ{isess}')';
    SZ_toplot{isess} = SZz(ind1,:);
    SZz_norip = zscore(SZ_norip{isess}')';
    SZ_norip_toplot{isess} = SZz_norip(ind1,:);
    % Other place cells
    OPCz = zscore(OPC{isess}')';
    OPC_toplot{isess} = OPCz(ind2,:);
    OPCz_norip = zscore(OPC_norip{isess}')';
    OPC_norip_toplot{isess} = OPCz_norip(ind2,:);
    % Other cells
    OCCz = zscore(OCC{isess}')';
    OCC_toplot{isess} = OCCz(ind3,:);
    OCCz_norip = zscore(OCC_norip{isess}')';
    OCC_norip_toplot{isess} = OCCz_norip(ind3,:);
end
...
if IsSave
    switch expeName
        ...
        case 'StimMFBWake'
            figName = ['PETHOnRipples_' num2str(mice)];
            dirPath = '/home/mobs/Dropbox/DataSL/StimMFBWake/Spikes/PETHonRipples/';
            saveF(f1,figName,dirPath,'sformat',{'dpng'},'res',300,'savfig',0)
            disp(['M' num2str(mice) ' saved']);
    end
end