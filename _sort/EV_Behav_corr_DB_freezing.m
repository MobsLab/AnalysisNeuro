function EV_4corr = EV_Behav_corr_DB_freezing(mice, expe, EV_epoch, varargin)
%
...
for imouse=1:length(Dir.path)
    for isession = 1:length(Dir.path{imouse})
        b{cnt} = load([Dir.path{imouse}{isession} 'behavResources.mat'], 'behavResources', 'FreezeAccEpoch', 'ZoneEpoch', 'SessionEpoch');
        try
...
% Occupancy
[rho, p] = corr(EV_4corr, OverallFreeze', 'Type', 'Spearman'); 
r(1) = rho;
pvalue(1) = p;

% Entries
[rho, p] = corr(EV_4corr, ShockFreeze', 'Type', 'Spearman'); 
r(2) = rho;
pvalue(2) = p;
...
subplot(121)
scatter(EV_4corr*100, OverallFreeze', 150, 'k', 'filled')
...
subplot(122)
scatter(EV_4corr*100, SafeFreeze', 150, 'k', 'filled')
...
if sav
    ...
    saveas(f,[foldertosave filesep 'EV' filesep 'Correlations' filesep 'CorrEV_freeze_' type_sleep '_' EV_epoch '.fig']);
    saveFigure(f,['CorrEV_freeze_' type_sleep '_' EV_epoch], [foldertosave filesep 'EV' filesep 'Correlations']);
end
