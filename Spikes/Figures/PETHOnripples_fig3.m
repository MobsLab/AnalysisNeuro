%% Parameters
IsSave=0;
mice = [797 798 828 861 882 905 906 911 912 977 994 1161 1162 1168 1182 1186];
wi = 0.5; % in sec
binsize = 0.01; % in sec
nbins = 100;
...
%% Group figures - box plots
cols = {[0.2 0.2 0.2], [0.8 0.2 0.2], [0.2 0.2 0.8]};

Pre = [SZ{1}; OCC{1}; OPC{1}];
Cond = [SZ{2}; OCC{2}; OPC{2}];
Post = [SZ{3}; OCC{3}; OPC{3}];
% All pooled
f2 = figure;
MakeBoxPlot_DB({mean(Pre(:,40:65)), mean(Cond(:,40:65)), mean(Post(:,40:65))}, cols, 1:3, titles, 0);
p_precond = ranksum(mean(Pre(:,40:65)),mean(Cond(:,40:65)));
if p_precond<0.05
    sigstar_DB({[1 2]},p_precond,0,'LineWigth',16,'StarSize',24);
end
p_prepost = ranksum(mean(Pre(:,40:65)),mean(Post(:,40:65)));
if p_prepost<0.05
    sigstar_DB({[1 3]},p_prepost,0,'LineWigth',16,'StarSize',24);
end
p_condpost = ranksum(mean(Cond(:,40:65)),mean(Post(:,40:65)));
if p_condpost<0.05
    sigstar_DB({[2 3]},p_condpost,0,'LineWigth',16,'StarSize',24);
end
ylabel('Z-scored firing rate around ripples')
makepretty
% Different groups
f3 = figure('units', 'normalized', 'outerposition', [0 0 1 0.5]);
subplot(131)
MakeBoxPlot_DB({mean(SZ{1}(:,40:65)), mean(SZ{2}(:,40:65)), mean(SZ{3}(:,40:65))}, cols, 1:3, titles, 0);
p_precond = ranksum(mean(SZ{1}(:,40:65)),mean(SZ{2}(:,40:65)));
if p_precond<0.05
    sigstar_DB({[1 2]},p_precond,0,'LineWigth',16,'StarSize',24);
end
p_prepost = ranksum(mean(SZ{1}(:,40:65)),mean(SZ{3}(:,40:65)));
if p_prepost<0.05
    sigstar_DB({[1 3]},p_prepost,0,'LineWigth',16,'StarSize',24);
end
p_condpost = ranksum(mean(SZ{1}(:,40:65)),mean(SZ{3}(:,40:65)));
if p_condpost<0.05
    sigstar_DB({[2 3]},p_condpost,0,'LineWigth',16,'StarSize',24);
end
title('Shock zone cells');
ylim([-.05 0.3])
ylabel('Z-scored firing rate around ripples')
makepretty
subplot(132)
MakeBoxPlot_DB({mean(OPC{1}(:,40:65)), mean(OPC{2}(:,40:65)), mean(OPC{3}(:,40:65))}, cols, 1:3, titles, 0);
p_precond = ranksum(mean(OPC{1}(:,40:65)),mean(OPC{2}(:,40:65)));
if p_precond<0.05
    sigstar_DB({[1 2]},p_precond,0,'LineWigth',16,'StarSize',24);
end
p_prepost = ranksum(mean(OPC{1}(:,40:65)),mean(OPC{3}(:,40:65)));
if p_prepost<0.05
    sigstar_DB({[1 3]},p_prepost,0,'LineWigth',16,'StarSize',24);
end
p_condpost = ranksum(mean(OPC{1}(:,40:65)),mean(OPC{3}(:,40:65)));
if p_condpost<0.05
    sigstar_DB({[2 3]},p_condpost,0,'LineWigth',16,'StarSize',24);
end
makepretty
ylim([-.05 0.3])
title('Other place cells');
subplot(133)
MakeBoxPlot_DB({mean(OCC{1}(:,40:65)), mean(OCC{2}(:,40:65)), mean(OCC{3}(:,40:65))}, cols, 1:3, titles, 0);
p_precond = ranksum(mean(OCC{1}(:,40:65)),mean(OCC{2}(:,40:65)));
if p_precond<0.05
    sigstar_DB({[1 2]},p_precond,0,'LineWigth',16,'StarSize',24);
end
p_prepost = ranksum(mean(OCC{1}(:,40:65)),mean(OCC{3}(:,40:65)));
if p_prepost<0.05
    sigstar_DB({[1 3]},p_prepost,0,'LineWigth',16,'StarSize',24);
end
p_condpost = ranksum(mean(OCC{1}(:,40:65)),mean(OCC{3}(:,40:65)));
if p_condpost<0.05
    sigstar_DB({[2 3]},p_condpost,0,'LineWigth',16,'StarSize',24);
end
...
if IsSave
    foldertosave = ChooseFolderForFigures_DB('Spikes');
    saveas(f2, [foldertosave '/Ripples/Groups_onRipples.fig']);
    saveFigure(f2, 'Groups_onRipples', [foldertosave '/Ripples']);
end
...
if IsSave
    foldertosave = ChooseFolderForFigures_DB('Spikes');
    saveas(f3, [foldertosave '/Ripples/SpikesonRipples_mean_groups.fig']);
    saveFigure(f3, 'SpikesonRipples_mean_groups', [foldertosave '/Ripples']);
    saveas(f4, [foldertosave '/Ripples/PieGroups.fig']);
    saveFigure(f4, 'PieGroups', [foldertosave '/Ripples']);
end
