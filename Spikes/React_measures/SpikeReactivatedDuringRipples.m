...
%%% SpikeReactivatedDuringRipples
% 
% This function builds a spatial map of the reactivated spikes
% 
% More precisely, once a spike is detected during a NREM SWR
% a place field is drawn on a map
% 
% Last touched on 09/04/2020
% 
...
for imouse = 1:length(Dir.path)
    ...
    ripprob_pc.Pre = flatten_cellarray(ripprob_pc.Pre, 1);
    ripprob_pc.Cond = flatten_cellarray(ripprob_pc.Cond, 1);
    ripprob_pc.Post = flatten_cellarray(ripprob_pc.Post, 1);
    
    coefs.Pre = numRip_pre * nanmean(ripprob_pc.Pre);
    coefs.Cond = numRip_cond * nanmean(ripprob_pc.Cond);
    coefs.Post = numRip_post * nanmean(ripprob_pc.Post);
